...
pub trait RefundTrait {
    /// 申请退款
    #[allow(dead_code)]
    fn refund(&self, data: ReqRefundOrder) -> impl Future<Output =  Result<RefundResponse, WeaError>>;
    /// 查询退款
    #[allow(dead_code)]
    fn query_refund(&self, out_refund_no: &str) -> impl Future<Output =  Result<RefundResponse, WeaError>>;
...
impl RefundTrait for Payment<WechatConfig> {
    fn refund(&self, data: ReqRefundOrder) -> impl Future<Output =  Result<RefundResponse, WeaError>>{
        ...
        async move {
            ...
            self.do_request::<RefundResponse>(&url, "POST", &refund_body).await
        }
    ...
    fn query_refund(&self, out_refund_no: &str) -> impl Future<Output =  Result<RefundResponse, WeaError>> {
        ...
        async move{
            self.do_request::<RefundResponse>(&url, "GET", "").await
        }
