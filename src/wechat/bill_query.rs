...
pub trait BillTrait {
    /// 申请交易账单
    #[allow(dead_code)]
    fn trade_bill(&self, bill_date: String,bill_type: Option<String>,tar_type:Option<String>,with_mchid: bool) -> impl Future<Output = Result<BillResponse, WeaError>>;
    /// 申请资金账单
    #[allow(dead_code)]
    fn fund_bill(&self, bill_date: String,account_type: Option<String>,tar_type:Option<String>) -> impl Future<Output = Result<BillResponse, WeaError>>;
    /// 下载帐单
    #[allow(dead_code)]
...
impl BillTrait for Payment<WechatConfig> {
    fn trade_bill(&self, bill_date: String,bill_type: Option<String>,tar_type:Option<String>,with_mchid: bool) -> impl Future<Output = Result<BillResponse, WeaError>> {
        let mut url = format!("/v3/bill/tradebill?bill_date={}&bill_type={}&tar_type={}",bill_date,bill_type.unwrap_or("ALL".to_string()),tar_type.unwrap_or("GZIP".to_string()));
        if self.is_sp() && with_mchid {
            url = format!("{}&sub_mchid={}",url,self.config.mchid.clone());
        }
        async move {
            self.do_request::<BillResponse>(&url, "GET", "").await
        }
    }
    fn fund_bill(&self, bill_date: String,account_type: Option<String>,tar_type:Option<String>) -> impl Future<Output = Result<BillResponse, WeaError>> {
        let url = format!("/v3/bill/fundflowbill?bill_date={}&account_type={}&tar_type={}",bill_date,account_type.unwrap_or("BASIC".to_string()),tar_type.unwrap_or("GZIP".to_string()));
        async move {
            self.do_request::<BillResponse>(&url, "GET", "").await
        }
...
mod tests{
    ...
    async fn test_fund_bill(){
        let config = crate::tests::get_config();
        let payment = Payment::new(config);
        let result = payment.fund_bill("2024-07-01".to_string(),Some("BASIC".to_string()),None).await;
        if result.is_err() {
            let error  = result.err().unwrap();
            println!("{}", error);
        }else{
